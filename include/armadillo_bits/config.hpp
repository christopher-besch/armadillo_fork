...
///lead to incorrect results / 2 = as per level 1, and warnings about poorly
///conditioned systems (low rcond) detected by solve(), spsolve(), etc / 3 = as
///per level 2, and warnings about failed decompositions, failed saving/loading,
///etc
...
//// Comment out the above line if you're getting linking errors when compiling
///your programs, / or if you prefer to directly link with LAPACK, BLAS + etc
///instead of the Armadillo runtime library. / You will then need to link your
///programs directly with -llapack -lblas instead of -larmadillo
...
///declaration) that have char arguments / (like many BLAS and LAPACK functions)
///also have associated "hidden" arguments. / For each char argument, the
///corresponding "hidden" argument specifies the number of characters. / These
///"hidden" arguments are typically tacked onto the end of function definitions.
...
///mkl_free() instead of standard malloc() and free()

// #define ARMA_USE_MKL_TYPES
//// Uncomment the above line if you want to use Intel MKL types for complex
///numbers. / You will need to include appropriate MKL headers before the
///Armadillo header. / You may also need to enable or disable the following
///options: / ARMA_BLAS_LONG, ARMA_BLAS_LONG_LONG, ARMA_USE_FORTRAN_HIDDEN_ARGS
...
///compiler supporting OpenMP 3.1 is detected.
#endif

#if !defined(ARMA_64BIT_WORD)
// #define ARMA_64BIT_WORD
//// Uncomment the above line if you require matrices/vectors capable of holding
///more than 4 billion elements. / Note that ARMA_64BIT_WORD is automatically
///enabled when std::size_t has 64 bits and ARMA_32BIT_WORD is not defined.
#endif

#if !defined(ARMA_USE_HDF5)
// #define ARMA_USE_HDF5
//// Uncomment the above line to allow the ability to save and load matrices
///stored in HDF5 format; / the hdf5.h header file must be available on your
///system, / and you will need to link with the hdf5 library (eg. -lhdf5)
#endif

#if !defined(ARMA_OPTIMISE_BAND)
#define ARMA_OPTIMISE_BAND
//// Comment out the above line if you don't want automatically optimised
///handling / of band matrices by solve() and chol()
#endif

#if !defined(ARMA_OPTIMISE_SYMPD)
#define ARMA_OPTIMISE_SYMPD
//// Comment out the above line if you don't want automatically optimised
///handling / of symmetric/hermitian positive definite matrices by various
///functions: / solve(), inv(), pinv(), expmat(), logmat(), sqrtmat(), rcond()
#endif
...
///Armadillo evaluates expressions. / This is mainly useful for debugging of the
///library.
...
#pragma message(                                                               \
    "WARNING: support for ARMA_DONT_USE_EXTERN_CXX11_RNG is deprecated and will be removed;")
...
///construction of dense matrices and cubes
#endif
